...
public class SimpleKMeansEx
  ...
  private int [] initialCentroids = null;
  
  public void setInitialCentroids(int [] initC)
  {
	  if (initC == null)
	  {
	  ...
	  initialCentroids = new int[initC.length];
	  for (int i = 0; i < initC.length; i++)
		  initialCentroids[i] = initC[i];
  }
...
public void buildClusterer(Instances data) throws Exception {
    ...
    if (initialCentroids == null){
    	// added by jing zhang
    	initialCentroids = new int[m_NumClusters];
    	int initialCounter = 0;
    	// end added by jingzhang
	  for (int j = initInstances.numInstances() - 1; j >= 0; j--) {
	      ...
	      if (!initC.containsKey(hk)) {
	        ...
	        // added by jing Zhang
	        initialCentroids[initialCounter] = instIndex;
	        initialCounter++;
...
public String[] getOptions () {
  ...
  public static void main (String[] argv) {
    runClusterer(new SimpleKMeansEx(), argv);
  }
}
